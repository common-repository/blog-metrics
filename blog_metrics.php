<?php
/*
Plugin Name: Blog Metrics
Version: 1.2.6
Plugin URI: http://yoast.com/wordpress/blog-metrics/
Description: Provides blog metrics based on the blog metrics Avinash Kaushik proposed in <a href="http://www.kaushik.net/avinash/2007/11/blog-metrics-six-recommendations-for-measuring-your-success.html">this post</a>, now with a widget!
Author: Joost de Valk
Author URI: http://yoast.com/

Copyright 2007-2009 Joost de Valk  (email : joost@yoast.com)
	...
	class BM_Admin {
		...
		function config_page() {
			...
			<ul class="pluginmenu">
				...
				<li><a href="http://wordpress.org/extend/plugins/blog-metrics/">Give it a good rating</a> on WordPress.org.</li>
				<li><a href="https://www.paypal.com/cgi-bin/webscr?cmd=_s-xclick&amp;hosted_button_id=3262694">Donate</a> a token of your appreciation!</li>
			</ul>
...
function bm_dashboard() {
	...
	echo '<td style="width:220px;"><h3 style="margin-bottom:0;">Last 30 days</h3></td>';
	echo '</tr>';
	echo '<tr>';
	bm_print_stats(bm_get_stats());
	bm_print_stats(bm_get_stats("month"));
	echo '</tr>';
	echo '</table>';
	$numauthorquery = "SELECT COUNT(DISTINCT p.post_author) FROM $wpdb->posts p";
	$numauthors = $wpdb->get_var($numauthorquery);
	if ($numauthors > 1) {
		echo '<h2 style="margin-top:20px;">Author stats for the last 30 days</h2>';
		bm_author_stats("month");
...
function widget_blog_metrics_control() {
	...
	// Defaults
		...
		if ( $_POST['blogmetricswidget-submit'] ) {
			...
			if (isset($_POST['blogmetricswidget-linkback'])) {
				$options['linkback']		= true;
			} else {
	...
	?>
		<table>
			...
			<tr>
				<th scope="row" style="text-align: right;">Link back to plugin page?</th>
				<td><input id="blogmetricswidget-linkback" name="blogmetricswidget-linkback" type="checkbox" <?php if ($options['linkback']) echo 'checked="chechked"'; ?> /></td>
			</tr>
...
function widget_blog_metrics_init() {
	...
	function widget_blog_metrics($args) {
		...
		echo '<li><ul>';
		if ($stats['avgposts'] == 1) {
			echo '<li>'.$stats['avgposts']." post $per month</li>\n";
		} else {
			echo '<li>'.$stats['avgposts']." posts $per month</li>\n";
		}
		...
		if ($options['linkback']) {
			echo '<li><br/><a href="http://yoast.com/wordpress/blog-metrics/">Blog Metrics</a> by Joost de Valk</li>'."\n";
		}
		echo "</ul>\n";
		echo $after_widget;
	}
